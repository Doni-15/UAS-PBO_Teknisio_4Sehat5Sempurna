...
> Update fokus: konfigurasi H2 Database, H2 Console, Java Trigger H2, strict API regression test final `382 passed, 0 failed`, dan perubahan status fitur yang sudah benar-benar diuji.

Roadmap pengerjaan backend **Teknisio** untuk aplikasi desktop dan mobile.

Dokumen ini menjadi panduan utama backend: prinsip pengembangan, flow MVP, status pengerjaan, kontrak API, prioritas sprint, dan aturan testing. Roadmap ini sudah disesuaikan dengan kondisi backend terbaru: flow auth, device category, technician skill, technician discovery, service request customer, service request technician, status history read API, update profile, create review, konfigurasi H2 Database, H2 Console, dan strict API regression test sudah terbentuk.
...
- Naik menjadi `[finished]` hanya jika sudah lolos test manual penting dan masuk `develop/api-strict-test.sh` atau strict regression test yang aktif.
...
[finished]    Konfigurasi H2 Database
[finished]    H2 Console development access
[finished]    Strict API regression test final: 382 passed, 0 failed
...
[finished]    Status history read API for customer and technician
[finished]    Update profile sendiri basic
[finished]    Review schema V6
[finished]    Customer create review
...
- Status history dan create review sudah dikunci melalui strict API regression test final.
- Strict API regression test terakhir: `382 passed, 0 failed`.
- `ReviewRepository` dan migration `V6__create_reviews.sql` sudah ada, jadi roadmap review tidak lagi dimulai dari nol.
- Database aktif untuk UAS desktop sekarang menggunakan **H2 file-based database**.
- `.gitignore` perlu memastikan file lokal seperti `.env`, `data/`, `build/`, `.idea/`, dan database H2 tidak ikut commit.
- `.gitignore` juga perlu dicek karena ignore `*.sh` dapat membuat script strict test tidak ikut commit.
...
↓
Data disimpan ke H2 Database melalui REST API Spring Boot + JPA/Hibernate
↓
...
- `JenisLayanan` dan `TeknisiLayanan` boleh dianggap legacy.
- Untuk MVP, flow utama tidak memakai `jenis_layanan`.
...
- `[finished]` Database aktif menggunakan H2 file-based database
- `[finished]` Hibernate schema generation dimatikan dengan `ddl-auto: none`
- `[finished]` `/actuator/health` status `UP`
- `[finished]` H2 Console bisa diakses untuk development
- `[finished]` Tidak ada error migration

Migration aktif untuk H2 Database:
...
- Dependency database aktif menggunakan H2.
- Datasource aktif memakai `jdbc:h2:file:./data/teknisio_db`.
- Flyway tetap menjadi sumber utama schema.
- Trigger database menggunakan Java Trigger H2.
- `ddl-auto` menggunakan `none` agar Hibernate tidak mengubah/menvalidasi schema H2 secara keliru.
...
## BE-06 [MVP] Konfigurasi H2 Database untuk UAS Desktop

Status: `[finished]`

Target: backend memakai H2 Database sebagai database runtime lokal sesuai syarat UAS, tanpa mengubah flow dan struktur bisnis utama.

Checklist:

- `[finished]` Dependency database menggunakan `com.h2database:h2`
- `[finished]` Flyway tetap aktif sebagai pengelola schema
- `[finished]` Datasource diarahkan ke H2 file-based database
- `[finished]` `.env` memakai `H2_URL`, `H2_USER`, dan `H2_PASSWORD`
- `[finished]` `application.yml` memakai `org.h2.Driver`
- `[finished]` `spring.jpa.hibernate.ddl-auto` diset ke `none`
- `[finished]` Migration SQL V1 sampai V6 kompatibel dengan H2
- `[finished]` UUID default memakai `RANDOM_UUID()`
- `[finished]` `TIMESTAMPTZ` diganti ke `TIMESTAMP WITH TIME ZONE`
- `[finished]` `ON CONFLICT` diganti dengan mekanisme H2-compatible
- `[finished]` Trigger database dibuat dengan Java Trigger H2
- `[finished]` H2 Console bisa dibuka di `/h2-console`
- `[finished]` `/actuator/health` return `UP`
- `[finished]` Flyway sukses apply V1 sampai V6
- `[finished]` Strict API regression test final lulus `382 passed, 0 failed`

File yang berhubungan dengan konfigurasi H2 dan pengujian:
...
src/main/resources/db/migration/V4__create_triggers.sql
src/main/resources/db/migration/V5__seed_device_categories.sql
src/main/resources/db/migration/V6__create_reviews.sql
src/main/java/com/teknisio/database/trigger/*
src/main/java/com/teknisio/config/SecurityConfig.java
```

Catatan:

- Controller, service, repository, DTO, entity utama, auth flow, dan business logic utama tidak dibongkar.
- Perubahan Java utama hanya penambahan Java Trigger H2 dan whitelist `/h2-console/**` untuk development.
- H2 Console hanya untuk development/demo lokal, bukan kontrak API client.
...
- `[finished]` Endpoint sudah ada
- `[finished]` Hanya customer login
- `[finished]` Customer hanya bisa melihat history request miliknya
- `[finished]` Validasi UUID
- `[finished]` Request bukan milik customer login return `404`
- `[finished]` Sort by `createdAt ASC`
- `[finished]` Response field English
- `[finished]` Sudah masuk strict API regression test final
...
- `[finished]` Endpoint sudah ada
- `[finished]` Request harus milik customer login
- `[finished]` Request harus `COMPLETED`
- `[finished]` Rating wajib 1 sampai 5
- `[finished]` Comment opsional
- `[finished]` Satu request hanya boleh satu review
- `[finished]` Update `ratingAvg` technician
- `[finished]` Update `ratingCount` technician
- `[finished]` Sudah masuk strict API regression test final
...
- `[finished]` Endpoint sudah ada
- `[finished]` Hanya technician login
- `[finished]` Technician hanya bisa melihat history request yang ditujukan kepadanya
- `[finished]` Validasi UUID
- `[finished]` Request milik technician lain return `404`
- `[finished]` Sort by `createdAt ASC`
- `[finished]` Response field English
...
- `[finished]` Endpoint customer tersedia
- `[finished]` Endpoint technician tersedia
- `[finished]` Customer hanya bisa melihat history request miliknya
- `[finished]` Technician hanya bisa melihat history request yang ditujukan kepadanya
- `[finished]` Sort by `createdAt ASC`
- `[finished]` Response field English
- `[finished]` Sudah masuk strict API regression test final sebagai BE-63

## BE-63 [MVP] Strict regression test status history

Status: `[finished]`

Target:

- `[finished]` Test customer get status history setelah create request
- `[finished]` Pastikan history punya `WAITING`
- `[finished]` Test setelah accept, ada `ACCEPTED`
- `[finished]` Test setelah start, ada `ON_PROGRESS`
- `[finished]` Test setelah complete, ada `COMPLETED`
- `[finished]` Test technician get status history
- `[finished]` Test wrong role `403`
- `[finished]` Test invalid UUID `400`
- `[finished]` Test not owner / not assigned `404`

BE-63 sudah lolos melalui strict API regression test final, sehingga BE-62 sudah naik menjadi `[finished]`.
...
- `[finished]` Update `ratingAvg` technician
- `[finished]` Update `ratingCount` technician
- `[finished]` Strict test create review success `201`
- `[finished]` Strict test review sebelum completed harus `409`
- `[finished]` Strict test duplicate review harus `409`
- `[finished]` Strict test wrong owner harus `404`
...
- `[finished]` Perubahan rating sudah ikut flow create review pada strict regression test

## BE-84 [MVP] Strict regression test review

Status: `[finished]`

Target test:

- `[finished]` Create service request sampai completed
- `[finished]` Customer create review success `201`
- `[finished]` Response punya `reviewId`, `rating`, `comment`
- `[finished]` Duplicate review return `409`
- `[finished]` Review request yang belum completed return `409`
- `[finished]` Review request milik customer lain return `404`
- `[finished]` Invalid rating return `400`
- `[finished]` Rating technician berubah setelah review

BE-84 sudah lolos melalui strict API regression test final. BE-81 sudah naik menjadi `[finished]`. BE-83 tetap perlu dicek lagi di sisi tampilan discovery/review list jika frontend membutuhkan validasi rating summary yang lebih eksplisit.
...
- `[ ]` Tambahkan ke strict regression test `develop/api-strict-test.sh`
...
Target terakhir yang sudah tercapai pada strict API regression test:
...
- Strict test ini sudah mencakup health, auth, validation, security role, device category, technician skill, technician discovery, service request customer, service request technician, status history, cancel, reject, complete, dan review.
- Status BE-44, BE-45, BE-56, BE-62, BE-81, dan BE-84 sudah dinaikkan sesuai hasil strict test.
...
[finished] BE-62 Status History Read API
[finished] BE-81 Create Review
[finished] Konfigurasi H2 Database
[finished] Strict API regression test final 382/0
```

Urutan paling aman dari posisi sekarang:

```text
1. BE-82 List review technician
2. BE-71 Technician update availability status
3. BE-72 Technician update profile/description
4. Refactor BE-70 update profile request Map -> DTO
5. Putuskan refresh token + logout server-side tetap deferred atau naik ke MVP+
...
- Status history dan create review sudah masuk strict regression test final.
...
- Jangan ignore `develop/api-strict-test.sh` jika script test perlu masuk repository.
...
- `*.sh` akan membuat script seperti `develop/api-strict-test.sh` tidak ikut commit.
...
# Jangan ignore semua script shell, karena strict test perlu di-commit
# *.sh
```
...
- `.env` tetap private dan tidak ikut commit.
- `.env.example` boleh ikut commit, tetapi password asli harus diganti placeholder seperti `change_me`.
...
[finished] Status history read API + strict regression test
[finished] Customer create review + strict regression test
[todo] Technician review list API
[todo] Technician availability update
[finished] Strict regression test final pass untuk API yang sudah tersedia
```
...
```text
[x] Konfigurasi H2 Database
[x] Pastikan Flyway V1 sampai V6 sukses di H2
[x] Pastikan `/actuator/health` return `UP`
[x] Pastikan H2 Console bisa login
[x] Update strict API regression test untuk status history
[x] Update strict API regression test untuk create review
[x] Jalankan `bash develop/api-strict-test.sh`
[x] Strict API regression test lulus `382 passed, 0 failed`
[x] Ubah BE-44, BE-45, BE-56, BE-62, BE-81, dan BE-84 menjadi `[finished]`
...
[ ] Pastikan `data/`, `.env`, `build/`, `.idea/`, dan file H2 lokal tidak ikut commit
[ ] Sanitasi `.env.example` agar tidak berisi password asli
```
