...
CREATE TABLE users (
  ...
  CONSTRAINT chk_users_no_telepon_format
    CHECK (REGEXP_LIKE(no_telepon, '^\+?[0-9]{10,15}$'))
);
...
CREATE TABLE teknisi_profile (
  id_teknisi_profile   UUID DEFAULT RANDOM_UUID() PRIMARY KEY,
...
CREATE TABLE kategori_layanan (
  id_kategori    UUID DEFAULT RANDOM_UUID() PRIMARY KEY,
...
CREATE TABLE permintaan_layanan (
  id_permintaan          UUID DEFAULT RANDOM_UUID() PRIMARY KEY,

  kode_permintaan        VARCHAR(50) DEFAULT (
    'REQ-' ||
    FORMATDATETIME(CURRENT_TIMESTAMP, 'yyyyMMdd') ||
    '-' ||
    UPPER(SUBSTRING(REPLACE(CAST(RANDOM_UUID() AS VARCHAR), '-', ''), 1, 8))
  ) NOT NULL UNIQUE,
  ...
  waktu_permintaan       TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
  waktu_diterima         TIMESTAMP WITH TIME ZONE,
  waktu_diproses         TIMESTAMP WITH TIME ZONE,
  waktu_selesai          TIMESTAMP WITH TIME ZONE,
  waktu_dibatalkan       TIMESTAMP WITH TIME ZONE,
  waktu_ditolak          TIMESTAMP WITH TIME ZONE,
...
CREATE TABLE riwayat_status (
  id_riwayat      UUID DEFAULT RANDOM_UUID() PRIMARY KEY,
  id_permintaan   UUID NOT NULL,
  diubah_oleh     UUID,
...
CREATE TABLE user_session (
  id_session          UUID DEFAULT RANDOM_UUID() PRIMARY KEY,
